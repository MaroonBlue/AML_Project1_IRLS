...
class LogisticRegression_with_IRLS:
    ...
    def IRLS_with_interactions(self, X, y, interaction_ids, max_iter=100, w_init = 1, d = 0.0001, tolerance = 0.001):
        ...
        if self.test_flag:
            acc_all = np.zeros(max_iter-1)
            tol_all = np.zeros(max_iter-1)
        ...
        for i in range(max_iter):
            ...
            if self.test_flag and i>0:
                acc_all[i-1] = accuracy_score(self.y_test, self.predict_test(self.X_test, B_new))
                tol_all[i-1] = tol
